    ...
    return (
        <VStack theme={theme} w="100%" h="100%" bgColor="background" justify="space-between" items="center" pv="lg" ph="xl">
            <VStack theme={theme} gap="lg" pv="xl" ph="lg" items="start">
                ...
                <Text theme={theme} variant="headlineSmall" color="onBackground">회원가입 해주세요</Text>
                <Input theme={theme} w="100%" h="48px" placeholder="이메일을 입력해주세요" />
                <Input theme={theme} w="100%" h="48px" placeholder="비밀번호를 입력해주세요" />
                <Input theme={theme} w="100%" h="48px" placeholder="비밀번호를 다시 입력해주세요" />
                <HStack theme={theme} pv="lg" gap="sm">
                    <Text theme={theme} variant="bodyMedium" color="onSurfaceVariant">계정이 있으신가요?</Text>
                    <Text theme={theme} variant="bodyMedium" color="primary" onClick={() => navigate('/login')}>로그인</Text>
                </HStack>
            </VStack>
            <Button theme={theme} w="100%" radius="md" pv="lg" variant="primary">
                <Text theme={theme} variant="titleMedium" color="onPrimary">회원가입</Text>
            </Button>
        </VStack>
    );
}
